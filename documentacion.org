...
** TODO Componente ~Teller~

En el proyecto hay un componente (~Teller~) que podéis añadir a cualquier ~Actor~

#+REVEAL: split

Este componente tiene una atributo ~Story~ (de tipo ~Text~) que contiene un texto que se mostrará cuando el jugador "hable" con el actor

#+REVEAL: split

Además, el componente se activa cuando estamos cerca y pulsamos ~Hablar~ (por defecto, la tecla =E=, configurable)
...
Recordad: intentad enviar lo que se ha hecho en clase, no es necesario dedicar mucho más tiempo a los niveles (para que le dediquéis el tiempo al proyecto)

(Que haya mucho plazo no significa que tengáis que dedicar mucho tiempo)
